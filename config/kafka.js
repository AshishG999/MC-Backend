...
async function initKafka() {
  ...
  kafka = new Kafka({
    clientId: process.env.KAFKA_CLIENT_ID || "microsite-backend",
    brokers,
  });
  ...
  try {
    ...
    logger.info(
      `Kafka topics created or verified: ${topics
        .map((t) => t.topic)
        .join(", ")}`
    );
  } catch (err) {
...
async function sendKafkaMessage(topic, message) {
  try {
    ...
    logger.error(`Kafka send error [${topic}]:`, err.message);
  }
}

module.exports = { initKafka, getProducer, sendKafkaMessage };
